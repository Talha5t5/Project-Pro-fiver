...
export class MySQLStorage implements IStorage {
  ...
  async getCollaboratorsByActivity(activityId: number): Promise<number[]> {
    ...
    return result.map((ac: any) => ac.collaboratorId);
  }
  ...
  async getActivitiesByCollaborator(collaboratorId: number): Promise<number[]> {
    ...
    return result.map((ac: any) => ac.activityId);
  }
  ...
  async assignCollaboratorToActivity(activityId: number, collaboratorId: number): Promise<ActivityCollaborator> {
    ...
    const newAssignment = await this.db.select().from(activityCollaborators).where(eq(activityCollaborators.activityId, activityId)).limit(1);
  ...
  async createPromotionalSpot(spot: InsertPromotionalSpot): Promise<PromotionalSpot> {
    ...
    let insertId = Array.isArray(result) ? result[0]?.insertId : (result as any)?.insertId;

    // Fallback: some drivers don't return insertId consistently; fetch last row
    if (!insertId) {
      try {
        const [last] = await this.db.select().from(promotionalSpots).orderBy(desc(promotionalSpots.id)).limit(1);
        if (last?.id) insertId = last.id;
      } catch (e) {
        // ignore and throw below if still not resolved
      }
    }

    const newSpot = insertId ? await this.getPromotionalSpot(insertId) : undefined;
  ...
  async createPlanConfiguration(config: InsertPlanConfiguration): Promise<PlanConfiguration> {
    ...
    const newConfig = await this.getPlanConfiguration(insertId);
  ...
  async upsertGeneralSettings(settings: InsertGeneralSettings): Promise<GeneralSettings> {
    ...
    if (existing) {
      ...
      await this.db.update(generalSettings).set(settings).where(eq(generalSettings.id, existing.id));
